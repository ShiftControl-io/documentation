---
title: 'App Discovery'
description: 'Detect shadow IT with ShiftControl App Discovery. Automatically find unauthorized OAuth apps your employees are using across your org.'
keywords: ['ShiftControl', 'app discovery', 'SaaS', 'OAuth', 'shadow IT', 'application management']
icon: 'radar'
---
import ModeScreenshot from "@site/src/components/ModeScreenshot";

<p style={{fontSize: "large"}}>Find every app your team is using — including the ones they didn't tell you about.</p>

## Overview

App Discovery is your shadow IT detection tool. It automatically surfaces applications your users have authorized via OAuth — apps that may not be officially managed or approved by your organization. Instead of relying on employees to self-report every tool they sign up for, App Discovery finds them for you so you can decide whether to manage, monitor, or block them.

<ModeScreenshot path="Apps/App-discovery" alt="App Discovery showing unauthorized OAuth apps detected across the organization" />
...
<ModeScreenshot path="Apps/App-Assignment-details" alt="App Discovery details" />
...
<ModeScreenshot path="Apps/App-discovery-bulk-actions" alt="App Discovery bulk actions" />
...
- Dismissing an app does not revoke OAuth access. It only removes the app from the Pending view. To actually revoke access, [block the app](/using-shiftcontrol/Apps/Blocking-apps) — that pulls existing grants and keeps new ones from sticking.

## Related Features

- [Blocking Apps](/using-shiftcontrol/Apps/Blocking-apps) -- block a discovered app across your Google Workspace and revoke everyone's access to it.
