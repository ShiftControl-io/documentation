---
title: 'Adding a User'
description: 'Add a user in ShiftControl to create their identity, provision app access, and automate onboarding across JumpCloud and connected tools.'
...
Adding a user in ShiftControl is the first step in onboarding. It's not just creating an account — it's establishing a person's entire digital identity across your organization. The information you enter here flows into your connected directories, which use it to provision application access, assign security policies, and organize users into the right groups.
...
1. **A directory identity is created.** The user's profile, including their name, email, and organizational details, is pushed to your connected directories immediately via API as the source of truth for authentication.
...
When both JumpCloud and Google Workspace are connected, the new user is created in both directories simultaneously. JumpCloud handles authentication and SSO, while Google Workspace provisions their Google account, email, and access to Google services. The user form includes a **Google Workspace** section with additional fields:

- **Recovery Email** -- The email Google will use if the user is locked out of their Google account.
- **Recovery Phone** -- A phone number for Google account recovery.
- **Show user in global directory** -- Whether this user appears in your organization's Google Workspace directory.
- **Email Aliases** -- Additional email addresses that route to the user's primary Google mailbox.
...
<ModeScreenshot path="Users/Add-a-user" alt="User actions" />
...
<Steps>
    ...
    <Step title="Enter identity information">
        ...
        - **First Name / Last Name** -- The user's legal name. This appears across all connected applications and directories.
    ...
    <Step title="Save the user">
        Click **Save Changes** to create the user. ShiftControl pushes the new identity to your connected directories immediately via API, evaluates group membership rules, provisions app access, and sends the activation email. The user will appear in the Users list with an **Inactive** status until they complete activation.
    </Step>
...
- **Primary Email cannot be easily changed later.** It becomes the user's SSO identity across all connected applications. Changing it after creation requires updates across every connected service. Get it right the first time.
